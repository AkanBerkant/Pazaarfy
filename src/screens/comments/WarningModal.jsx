...
import Other from "../BablContent/Other";

const WarningModal = ({ replyId, commentId, visible, onClose }) => {
  ...
  const onSendPress = (text) => {
    if (replyId) {
      reportMutation.mutate({ title: text, reply: replyId });
    } else if (commentId) {
      reportMutation.mutate({ title: text, comment: commentId });
    }
  };
